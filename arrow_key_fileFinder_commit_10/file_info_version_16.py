        ...
        if ops == "Windows":
            ...
            for i, drive in enumerate(drives, start=1):
                ...
                for drive in drives:
                    ...
                    if enable_file_ext_count_in_scan == "true"  :  
...
#                             total_n_xlsx=VALUES(total_n_xls), 
...
# #End - Old code 
        ...
        #the below is for windows
            ...
            insert_query = '''
                    ...
                    VALUES (
                        %(hostname)s, %(ip_address)s, %(os_name)s, %(os_version)s, %(system_info)s, %(number_of_drives)s, %(name_of_drives)s, 
                        %(total_n_files)s, %(total_n_xls)s, %(total_n_xlsx)s, %(total_n_doc)s, %(total_n_docx)s, %(total_n_pdf)s, %(total_n_zip)s, %(total_n_sql)s, %(total_n_bak)s, 
                        FROM_UNIXTIME(%(start_time)s), %(row_created_by)s, FROM_UNIXTIME(%(start_time)s), %(row_modification_by)s
                    ) ON DUPLICATE KEY UPDATE
                            total_n_files=VALUES(total_n_files), 
                            total_n_xls=VALUES(total_n_xls), 
                            total_n_xlsx=VALUES(total_n_xls), 
                    ...
                    '''
            data = (
                hostname, ipaddress, os_name, os_version, system_info_str, str(len(drives)), drive_names, 
                total_n_files, total_n_xls, total_n_xlsx, total_n_doc, total_n_docx, total_n_pdf, total_n_zip, total_n_sql, total_n_bak,
                start_time, employee_username, start_time, employee_username
