...
def upsert_to_database(file_path, connection, employee_username, start_time):
    ...
    cursor.execute('''
        INSERT INTO d_file_details (f_machine_files_summary_count_fk, ip_address,hostname,file_path, file_size_bytes, file_name, file_extension,file_owner,file_creation_time, file_modification_time, file_last_access_time, row_creation_date_time, row_created_by,row_modification_date_time,row_modification_by)
        VALUES ((SELECT f_machine_files_summary_count_pk FROM f_machine_files_summary_count WHERE hostname = %s), %s,%s,%s, %s, %s, %s,%s, %s, %s, %s,FROM_UNIXTIME(%s),%s,FROM_UNIXTIME(%s),%s)
...
def create_xls_file_sheet_table(connection, xls_files,employee_username, start_time):
    try:
        ...
        logger.error(f"Error creating .xls file tables and inserting data: {str(e)}", exc_info=True)
...
def create_xls_file_sheet_row_table(connection, xls_files,employee_username, start_time):
    try:
        ...
        logger.error(f"Error creating .xls file row tables and inserting data: {str(e)}", exc_info=True)
...
def create_audit_table(connection, employee_username, ip, start_time, end_time, elapsed_time, scan):
    ...
    try:
        ...
        logger.error(f"Error creating audit table and inserting data: {str(e)}", exc_info=True)
