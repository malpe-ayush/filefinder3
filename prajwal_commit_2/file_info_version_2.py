...
def create_xls_file_sheet_table(connection, xls_files):
    try:
        ...
        for xls_file in xls_files:
            ...
            workbook = pd.read_excel(xls_file, sheet_name=None)
...
# function for xlsx 
def create_xlsx_file_sheet_table(connection, xlsx_files):
    try:
        cursor = connection.cursor()
        for xlsx_file in xlsx_files:
            ...
            for sheet_name in sheets:
                #sheet = workbook.sheet_by_name(sheet_name)
                num_rows = sheet_name.max_row
                num_cols = sheet_name.max_col
                ...
                ''', (xlsx_file, sheet_name, num_cols, num_rows))
                connection.commit()
        print("Tables for .xls files created and data inserted.")
    except Exception as e:
        traceback.format_exc()
...
#function for xlsx sheet rows
def create_xlsx_file_sheet_row_table(connection, xlsx_files):
    try:
        cursor = connection.cursor()
        for xlsx_file in xlsx_files:
            xlsx_data = openpyxl.load_workbook(xlsx_file) # Read all sheets
            sheets = xlsx_data.sheetnames
            for sheet_name in sheets:
                #sheet = xlsx_data.sheet_by_name(sheet_name)
                num_rows = sheet_name.max_row
                num_cols = sheet_name.max_col
                ...
                for row_idx in range(min(int(os.getenv("MIN_ROW")), num_rows)):  # Read up to the first 3 rows
                    is_row = "no" if row_idx == 0 else "yes"  # First row is a heading, the rest are data
                    col_data = sheet_name.iloc[row_idx, :10].tolist()  # Take the first 10 columns
                    col_data.extend(["NULL"] * (10 - len(col_data)))  # Fill the remaining columns with "NULL"
                    col_data = [str(data)[:255] for data in col_data]  # Truncate data if necessary
                    # Check for truncation if there are more than 10 columns
                    is_truncate = "yes" if num_cols > 10 else "no"
...
if __name__ == "__main__":
    ...
    if not drives:
        ...
        if ".xls" in file_extensions:
            ...
            xlsx_files=[file for file in found_assets if file.lower().endswith(".xlsx")]
            if xlsx_files:
                connection = mysql.connector.connect(
                    host=host,
                    port=port,
                ...
                create_xlsx_file_sheet_table(connection, xlsx_files)
                create_xlsx_file_sheet_row_table(connection, xlsx_files)
                connection.close()
    end_time = time.time()
