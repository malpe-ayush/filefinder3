...
def upsert_to_database(file_path, connection):
    ...
    cursor.execute('''
        INSERT INTO file_name_info (machine_info_for_search_fk, ip_address,hostname,file_path, file_size, file_name, file_extension, modification_time, access_time, creation_time)
        VALUES ((SELECT machine_info_for_search_pk FROM machine_info_for_search WHERE hostname = %s), %s,%s,%s, %s, %s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE
        file_size = %s, file_name = %s, file_extension = %s, modification_time = %s, access_time = %s, creation_time = %s;
    ''', (
    hostname, ipaddrs, hostname, file_path, file_size, file_name, file_extension, modification_time, access_time, creation_time,
...
def create_audit_table(connection,EmployeeUserName, ip, start_time, end_time, elapsed_time):
    try:
        ...
        cursor.execute('''
            INSERT INTO audit_info (pc_ip_address ,employee_username, start_time,end_time, duration)
            VALUES (%s, %s, FROM_UNIXTIME(%s), FROM_UNIXTIME(%s), %s);
        ''', (ip, EmployeeUserName ,start_time, end_time, end_time - start_time))
        connection.commit()
...
def insert_machine_info_for_search(connection,ipaddress,hostname):
    try:
        cursor = connection.cursor()
        cursor.execute('''
                INSERT INTO machine_info_for_search (hostname ,ip_address)
                VALUES (%s, %s);
            ''', (hostname, ipaddress))
        connection.commit()
...
def windows():
    ...
    if not drives:
        ...
        try:
            ...
            insert_machine_info_for_search(connection,ipaddrs,hostname)
