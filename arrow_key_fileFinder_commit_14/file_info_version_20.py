...
#meta information for the script.
if getattr(sys, 'frozen', False):  # if the script is run as a bundled executable
    # Set metadata attributes
    sys.set_attribute('version', '2.0.0')
    sys.set_attribute('client', 'Lisney')
    sys.set_attribute('company', 'ie.gt.com')
    sys.set_attribute('author', 'Arunkumar')
    sys.set_attribute('email', 'Arunkumar.nair@ie.gt.com')
...
def upsert_to_database(file_path, connection, employee_username, start_time):
    try:
        ...
        truncated_file_path = file_path[:499]
...
# def app_log_to_db(app_log,connection,ipaddrs,hostname,start_time):
#     try:
#         cursor=connection.cursor()
#         with open(app_log, 'r') as f:
#             combined_error="\n".join(f.readlines())
#             cursor.execute('''
#             INSERT INTO app_log_file (f_machine_files_summary_count_fk,ip_address,hostname,app_log, row_creation_date_time, row_created_by,row_modification_date_time,row_modification_by)
#             VALUES ((SELECT f_machine_files_summary_count_pk FROM f_machine_files_summary_count WHERE ip_address= %s),%s,%s,%s,FROM_UNIXTIME(%s),%s,FROM_UNIXTIME(%s),%s
#             );
#         ''', (ipaddrs, ipaddrs, hostname, combined_error,start_time, employee_username, start_time, employee_username
#         ))  
#         connection.commit()
#         print("[bright_green]Data inserted into app_log table.[/bright_green]")
#         logger.success("Data inserted into app_log table.")
#     except Exception as e:
#         logger.error(f"Error app_log_to_db: {str(e)}", exc_info=True)
